...
{if $value}
    ...
    {else}
        <a class="link link-related-record" href="{$app_url}downloadManager/download/{$value}"><input class="form-control" type="text" name="{$filename_field}" value="{$value_nome}" {$ext_attrs} disabled="true" /></a>
    {/if}
{else}
    <input class="form-control" type="text" name="{$filename_field}" value="{$value_nome}" {$ext_attrs} disabled="true" />
{/if}
</p>